...
static void ToMergeParams(const TArray<FSkelMeshMergeUVTransformMapping>& InUVTransformsPerMesh, TArray<FSkelMeshMergeUVTransformMapping>& OutUVTransformsPerMesh)
{
    if (InUVTransformsPerMesh.Num() > 0)
    {
        ...
        {
            TArray<FSkelMeshMergeMeshUVTransforms>& OutUVTransforms = OutUVTransformsPerMesh[i].UVTransformsPerMesh;
            const TArray<FSkelMeshMergeMeshUVTransforms>& InUVTransforms = InUVTransformsPerMesh[i].UVTransformsPerMesh;
            if (InUVTransforms.Num() > 0)
            {
                ...
                {
                    OutUVTransforms[i].UVTransforms = InUVTransforms[i].UVTransforms;
                }
            }
        }
    }
...
USkeletalMesh* UMeshMergeFunctionLibrary::MergeMeshes(const FSkeletalMeshMergeParams& Params)
{
    ...
    TArray<FSkelMeshMergeUVTransformMapping> UvTransforms;
