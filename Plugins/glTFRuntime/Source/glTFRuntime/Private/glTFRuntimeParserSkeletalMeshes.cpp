...
void FglTFRuntimeParser::CopySkeletonLocationsFrom(FReferenceSkeleton& RefSkeleton, const FReferenceSkeleton& SrcRefSkeleton)
{
	FReferenceSkeletonModifier Modifier = FReferenceSkeletonModifier(RefSkeleton, nullptr);
	...
	TMap<FString, FTransform> SourceTransforms;

	for (int32 i = 0; i < SrcBonesTransforms.Num(); i++)
	{
		SourceTransforms.Add(SrcRefSkeleton.GetBoneName(i).ToString(), SrcBonesTransforms[i]);
	}
	

	for (const TPair<FString, FTransform>& Pair : SourceTransforms)
	{
		const int32 BoneIndex = RefSkeleton.FindBoneIndex(*Pair.Key);
		if (BoneIndex <= INDEX_NONE)
		{
		...
		FTransform Transform = BonesTransforms[BoneIndex];
		Transform.SetLocation(Pair.Value.GetLocation());
		Modifier.UpdateRefPoseTransform(BoneIndex, Transform);
	}
	...
	//	if (SrcBoneIndex > INDEX_NONE)
	//	{
	//		NewTransform.SetLocation(SrcBonesTransforms[SrcBoneIndex].GetLocation());
	//		int32 SrcParentIndex = SrcRefSkeleton.GetParentIndex(SrcBoneIndex);
	//		FVector3d AllLocations = FVector3d::ZeroVector;
	//		while (SrcParentIndex > INDEX_NONE)
	//		{
	//			AllLocations = SrcBonesTransforms[SrcParentIndex].GetLocation() * AllLocations;
	//			SrcParentIndex = SrcRefSkeleton.GetParentIndex(SrcParentIndex);
	//		}
	//		NewTransform.SetLocation(AllLocations);
	//		Modifier.UpdateRefPoseTransform(BoneIndex, NewTransform);

	//		UE_LOG(LogTemp, Warning, TEXT("%d_%d_%s"), SrcBoneIndex, SrcParentIndex, *AllLocations.ToString());
	//	}
	//}
}
...
USkeletalMesh* FglTFRuntimeParser::CreateSkeletalMeshFromLODs(TSharedRef<FglTFRuntimeSkeletalMeshContext, ESPMode::ThreadSafe> SkeletalMeshContext)
{
	...
	if (SkeletalMeshContext->SkeletalMeshConfig.SkeletonConfig.CopyLocationsFrom)
	{
		CopySkeletonLocationsFrom(RefSkeleton, SkeletalMeshContext->SkeletalMeshConfig.SkeletonConfig.CopyLocationsFrom->GetRefSkeleton());
	}
